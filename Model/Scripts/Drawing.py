...
class MainAI:
    def __init__(self):
        ...
        self.mask_img = cv2.imread('../Images/Mask/DogMask.jpg', cv2.IMREAD_GRAYSCALE)
        self.background_img = cv2.imread('../Images/Background/Background-white.jpg', cv2.IMREAD_COLOR)
        self.masking = ImageMasking()
        self.drawing = PetDrawing()
        self.masked_img = None


    def set_mask_img(self):
        self.masked_img = self.masking.mask_img(self.origin_img, self.mask_img, self.background_img)


    def main(self):
        self.set_mask_img()

        # result_img = self.drawing.k_mean_method(self.masked_img, 36, 1)
        # result_img = self.drawing.cartoon_method(self.masked_img)
        result_img = self.drawing.canny_method(self.masked_img)
